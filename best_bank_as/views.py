...
def search_customer(request: HttpRequest) -> HttpResponse:
	...
	context = {"customers": customers, "query": query, "status_list": status_list, "rank_list": rank_list}
...
def customer(request: HttpRequest, pk: int) -> HttpResponse:
	...
	if request.method == "PUT" and request.user.is_staff:
		...
		try:
			...
			messages.error(request, f'Some went wrong. Please try again')
		
		context = {"customer": customer, "rank_list": rank_list}
		return render(
			request, "best_bank_as/customers/customer_rank_partial.html", context,
		)
